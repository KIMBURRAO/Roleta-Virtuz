...
select ok(has_column_privilege('anon', 'public.prizes', 'name', 'SELECT'), 'anon can select public prize columns');
select ok(not has_column_privilege('anon', 'public.prizes', 'forced_at_spin', 'SELECT'), 'anon cannot select forced rule columns');
...
insert into public.prizes (id, name, color, initial_stock, current_stock, weight, active, forced_at_spin)
values ('00000000-0000-0000-0000-000000000101', 'Prêmio de teste', '#08C900', 1, 1, 1, true, 1);

insert into public.prizes (id, name, color, initial_stock, current_stock, weight, active)
values ('00000000-0000-0000-0000-000000000102', 'Prêmio normal', '#064F25', 1, 1, 10000, true);
...
select is((select prize_id from public.spins where client_spin_id = '00000000-0000-0000-0000-000000000201'), '00000000-0000-0000-0000-000000000101'::uuid, 'forced rule wins before weighted draw');
